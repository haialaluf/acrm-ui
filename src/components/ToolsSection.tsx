...
function GoogleMCPClientEditor<T extends FieldValues>({
  ...
  const handleGetToken = () => {
    ...
    const authUrl = `https://mcp.delacrm.com/auth/google?products=${product}&callback=${encodeURIComponent(callbackUrl)}`;
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === "oauth-callback" && event.data?.apiKey) {
        ...
        const updatedTool: LocalMCPToolConfig = {
          ...
          label: freshLabel,
          config: {
            url: event.data.url || "https://mcp.delacrm.com/mcp",
