...
# src/supabase/types/ with their source in acrm-api/_shared/types/.
#
...
# Env:    API_TYPES_DIR  (default: ../acrm-api/supabase/functions/_shared/types)
...
if [ ! -d "$API_DIR" ]; then
  ...
  echo "   Set API_TYPES_DIR to the acrm-api _shared/types path and re-run."
  exit 0
fi
